...
def simulator(chem_data, reactor_data, feed_data):
    ...
    dl = L / N
    col_list = ["z", "T", "P"] + ['F' + temp for temp in chem_data["comp_list"]]  # "q", "cp","dT_diff","dF_diff"
    ...
    for i in range(N):
        # calculate the diffusional flux across the insulator
        if insulator_para["status"] == "on":
            dF_diff, dT_diff = ins.mflux(T[0], P, F_pd.iloc[0], insulator_para)
            F_pd.loc[1, "H2O"] -= abs(dF_diff * dl * nit)
            T[0] -= abs(dT_diff * dl * nit)
        # dT_diff = dT_diff * dl

        sim_data.iloc[i, 1:3] = [T[0], P0]  # q[0], cp, dT_diff, dF_diff * dl
        sim_data.iloc[i, 3:] = F_pd.iloc[0]
...
path = "result_recycle_%s_%s_%s_%s_%s_%s_%s.xlsx" % \
       (feed_dict["condition"]["T"], feed_dict["condition"]["P"],
        reactor_dict["reactor"]["Dt"], reactor_dict["reactor"]["nt"],
        reactor_dict["insulator"]["status"], reactor_dict["insulator"]["Din"], reactor_dict["insulator"]["nt"])
