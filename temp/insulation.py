...
class Test:
    ...
    def mflux(self, i, j):
        ...
        for u in np.linspace(-5e-2, 0, 1000):
            c_nc_in_slope = self.ode(self.T1, self.T2, c_in - c_c_in, c_out - c_c_out, thick, u)[1][-1]
        ...
        na_mass_dif = -1 * 0.1e-4 * self.ode(self.T1, self.T2, c_c_in, c_c_out, thick, u_in)[1][-1] * self.mass[i]
...
# print(tes.mflux(0, [2, 3]))

n = 0
for T in Ts:
    tes = Test(523, T, 6e6, fluids, q_mass_in, x_conversion)
    h_flux[n], mass_flux[n], deviation[n] = tes.mflux(0, [2, 3])#[0:2]
