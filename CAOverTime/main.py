...
frequency = 400  # 2200  # 3200  # 2200  # 1400  # 2200  # 1400
equilibrium = 0  # 200  # 200
drop_type = -1  #

sl_types = [0.1260, 0.1449, 0.1890, 0.1575, 0.2520, 0.3150]  # Ture for volatile F for nonvolatile
vl_types = 0.0315
sv_types = 0.2835
...
root_path = 'D:/document/00Study/01接触角摩擦力/计算/液滴/stage8.2.6/LGV_001'
square_path = '%s/ar-square-170-%.4f-%.4f-%.4f.profile' % (root_path, sl_types[drop_type], vl_types, sv_types)
y_path = '%s/ar-y-170-%.4f-%.4f-%.4f.profile' % (root_path, sl_types[drop_type], vl_types, sv_types)

# square_path = '%s/ar2-square-100-%.4f.profile' % (root_path, sl_types[drop_type])
